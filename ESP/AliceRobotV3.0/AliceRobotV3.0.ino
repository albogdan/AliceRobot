...
//    rtc.adjust(DateTime(2019, 3, 25, 8, 19, 0));
...
/*****************BEGIN PROGRAM LOGIC****************/    
    if(DeviceMode == MODE_RUNNING){
        VL53L0X_RangingMeasurementData_t measureStart, measureTire;
...
//            sensorStepperMoving = true;
            ...
            if(tiresToAdd == 1){
                ...
                if(currentPole == 0){
                    //Add a tire
                    conveyorPosition = CONVEYOR_START_TIRE_WIDTH;
            ...
            Serial.write("MSL\n");
            //Delay an amount (same as before), might have to make it based on the distance of the robot from the measurement of the laser sensor
            delay(1000);
            //Turn off motor move
            Serial.write("MFF\n");
            delay(100);
        ...
        if(totalDistance >= 4000.0 || tiresLeft <= 0){
            //go to start
            Serial.write("MBB\n"); //Move back home
            long int i = 0;
            while(1){
