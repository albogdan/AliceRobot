...
void setup() {
    ...
    //rtc.adjust(DateTime(2019, 4, 9, 13, 57, 0));
    ...
    stepperConveyor.setMaxSpeed(2250.0);
    stepperConveyor.setAcceleration(200.0);
    stepperConveyor.setSpeed(1700);
...
/*****************BEGIN PROGRAM LOGIC****************/   
    if(DeviceMode == MODE_RUNNING){
        ...
        if(measureStart.RangeStatus!=4 && (measureStart.RangeMilliMeter > 100 && measureStart.RangeMilliMeter < 500)){
            ...
            distanceToBack = measureStart.RangeMilliMeter;
        }
        
        //If object(pole) detected then:
        if(poleDetected && !runComplete){
            Serial.print("D2Pole Detected!\n");
            delay(60);
            //Stop motors
            ...
            while(1){
                ...
                Serial.print("D4S:" + String(movingAvg)+ '\n');
                delay(100);
                Serial.write("MFF\n");
                delay(100);
                Serial.write("MSR\n");
 ...
 /*******NOTE THERE WILL BE A BUG - HAVE TO ALSO CONSIDER NUMBER OF TIRES CURRENTLY AVAILABLE TO DEPLOY******/
        ...
        if((totalDistance >= 2000.0 ||  (currentPole >=3 && tiresLeft<=4) || emergency) && !runComplete){
            //go to start
            //if(!emergency){
                Serial.write("MFF\n");
                delay(100);
