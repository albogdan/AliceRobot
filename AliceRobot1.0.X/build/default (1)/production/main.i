        ...
        switch(currentScreen->cursorLine){
            case 2:
                *currentScreen = runningOperation;
                ...
                sprintf(temp, "%02x:%02x:%02x", rtc.hour, rtc.min,rtc.sec);
...
void printDateTime(Screen *currentScreen){
    ...
    if(currentScreen->name == "HOME"){
        LCD_set_cursor(0,0);
        sprintf(tempTime, "%02x:%02x:%02x", rtc.hour, rtc.min,rtc.sec);
        currentScreen->lineText[0] = tempTime;

        sprintf(tempDate, "%02x/%02x/20%02x", (rtc.date)&0x0F+0x30, (rtc.month)&0x0F+0x30,(rtc.year)&0x0F+0x30);
